...
GESTURE_TO_NUMBER = {
    ...
    "M1": 5,  # 소축 재장전2 [-1,1,1,-1,-1] -> 1 번호 사용
}

# CSV 저장 관련 전역 변수
CSV_FOLDER = "gesture_data"  # CSV 파일을 저장할 폴더
CSV_FILE_PATH = None  # 실행 시 생성되는 CSV 파일 경로 (타임스탬프 포함)
last_saved_gesture = None  # 마지막으로 저장한 제스처 (중복 저장 방지)

# 엄지-다른손가락 접촉 임계값 (픽셀 거리 기준)
THUMB_TOUCH_THRESHOLD = 40
...
def classify_finger_state_single_angle(
    ...
    if angle_bottom is not None:
        # 하단 카메라 임계값: 소지 165도, 나머지 170도
        bottom_straight_threshold = 165 if finger_name == "Pinky" else 170
...
def classify_gesture_from_pattern_stabilized(
    ...
    if candidate_gesture != current_gesture_candidate:
        global last_saved_gesture
        current_gesture_candidate = candidate_gesture
        gesture_start_time = current_time
        # 새로운 제스처 후보이므로 CSV 저장 상태 초기화
...
def classify_gesture_from_pattern(
    ...
    if pattern == [-1, 1, 1, 1, 1]:
        ...
        # 검지-중지 거리 확인 (붙어있으면 S1, 벌어지면 4)
        index_middle_together = None
        if bottom_camera_state is not None:
            index_middle_together = getattr(
                bottom_camera_state, "index_middle_together", None
            )

        if index_middle_together is not None:
            if index_middle_together:
                return "S1"
            else:
                return "S1"  # 검지-중지 벌어지면 인식하지 않음 (4 제스처)
        else:
            # 거리 정보가 없으면 일단 S1로 처리
            return "S1"
    ...
    if pattern == [1, 1, 1, 1, 1]:
        ...
        if index_middle_together is not None:
            if index_middle_together:
                ...
                return "B"  # 검지-중지 벌어지면 인식하지 않음
        else:
            return None  # 거리 정보가 없으면 인식하지 않음
...
def process_hand_landmarks(
    ...
    if camera_state.camera_type == "bottom":
        try:
            ...
            # 엄지-검지 접촉이 필요한 패턴인지 확인
            needs_thumb_touch = (
                pattern == [0, 0, 0, 0, 0]  # C/O/Bent5
                or pattern == [0, 0, 1, 1, 1]  # F/OpenF
                or pattern == [0, 0, -1, -1, -1]  # BentL/BabyO
            )

            if needs_thumb_touch:
                thumb_touch_info = check_thumb_touching_fingers(
                    camera_state, hand_size_3d
                )
                camera_state.thumb_touch_info = thumb_touch_info
            else:
                camera_state.thumb_touch_info = None
...
def draw_results(
    ...
    # 상단 카메라(camera_id == 0)는 디버깅 정보 표시 안함
    # 하단 카메라도 디버깅 정보 제거 (MIDDLE CLASSIFICATION DETAILS, THUMB TOUCH STATUS)
    if camera_id == 0:
        pass  # 통합 상태만 표시 (아래에서 처리)
    ...
    if (
        ...
        # 제스처가 인식되면 CSV에 저장
        if gesture and bottom_camera_state and bottom_camera_state.hand_landmarks:
            save_gesture_to_csv(gesture, bottom_camera_state.hand_landmarks)

        # 제스처 표시 제거 (화면에 표시하지 않음)
        # if gesture:
        #     pass  # 제스처 인식은 되지만 화면에 표시하지 않음

    # 측면 카메라: 엄지가 손가락 사이에 끼어있는지 표시 제거
    # if (
    #     camera_id != 0
    #     and camera_state is not None
    #     and camera_state.camera_type == "side"
    # ):
    #     pass  # 모든 디버그 표시 제거
    #     # y0 += 30
    #     # 상세 값 표시 등 모두 제거됨
...
def init_csv_file():
    """CSV 파일을 초기화합니다. 매 실행마다 새로운 파일을 생성합니다."""
    global CSV_FILE_PATH

    # 폴더가 없으면 생성
    if not os.path.exists(CSV_FOLDER):
        os.makedirs(CSV_FOLDER)
        print(f"CSV 저장 폴더 생성: {CSV_FOLDER}")

    # 타임스탬프를 포함한 파일명 생성
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    CSV_FILE_PATH = os.path.join(CSV_FOLDER, f"gesture_landmarks_{timestamp}.csv")

    # 새 파일 생성 및 헤더 작성
    with open(CSV_FILE_PATH, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        # 헤더 작성: 제스처, 타임스탬프, 21개 랜드마크의 x, y, z 좌표
        header = ["gesture", "timestamp"]
        for i in range(21):
            header.extend([f"landmark_{i}_x", f"landmark_{i}_y", f"landmark_{i}_z"])
        writer.writerow(header)

    print(f"새 CSV 파일 생성: {CSV_FILE_PATH}")


def save_gesture_to_csv(gesture_name, hand_landmarks):
    """제스처와 랜드마크를 CSV 파일에 저장합니다."""
    global last_saved_gesture

    # 중복 저장 방지: 같은 제스처는 한 번만 저장
    if gesture_name == last_saved_gesture:
        return
    ...
    if gesture_name not in basic_gestures:
        return
    ...
    try:
        with open(CSV_FILE_PATH, "a", newline="", encoding="utf-8") as f:
            ...
            # 행 데이터 준비
            row = [gesture_name, timestamp]

            # 21개 랜드마크의 x, y, z 좌표 추가
            for landmark in hand_landmarks.landmark:
                row.extend([landmark.x, landmark.y, landmark.z])

            writer.writerow(row)
            print(f"CSV 저장: {gesture_name}")

            # 마지막 저장 제스처 업데이트
            last_saved_gesture = gesture_name
